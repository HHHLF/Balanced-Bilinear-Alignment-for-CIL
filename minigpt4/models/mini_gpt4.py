...
class MiniGPT4(Blip2Base):
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            if self.has_qformer: # Qformer
                ...
                self.act['llama_proj'] = query_output.last_hidden_state
                self.act_list.append(self.act['llama_proj'])
                if len(self.act_list) > 40:
                    self.act_list.pop(0)

                inputs_llama = self.llama_proj(query_output.last_hidden_state) ### 2,32,768 -> 2,32,4096 ### learnable ###
            else:
            ...
            atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(image.device) # 2,32
            qo_lhs = query_output.last_hidden_state
        return inputs_llama, atts_llama, qo_lhs
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts):
        if prompts:
            ...
            for each_img_embed, each_prompt in zip(img_embeds, prompts):
                ...
                wrapped_emb = torch.cat([p_before_embed, each_img_embed[None], p_after_embed], dim=1) # 1,53,4096
                emb_lists.append(wrapped_emb)
            emb_lens = [emb.shape[1] for emb in emb_lists]
            pad_emb = self.embed_tokens(torch.tensor(self.llama_tokenizer.pad_token_id, device=img_embeds.device))
            wrapped_embs = pad_emb.expand(len(emb_lens), max(emb_lens), -1).clone() # 2,53,4096
            wrapped_atts = torch.zeros([len(emb_lens), max(emb_lens)], dtype=torch.int, device=img_embeds.device) # 2,53
    ...
    def forward(self, samples):
        ...
        # import pdb; pdb.set_trace()
        image = samples["image"] # [2,3,224,224]
        img_embeds, atts_img, qo_lhs= self.encode_img(image) # [2,32,4096] [2,32]
        ...
        bos_embeds = self.embed_tokens(bos) # 2,1,4096
        atts_bos = atts_img[:, :1] # 2，1

        to_regress_embeds = self.embed_tokens(to_regress_tokens.input_ids) # 2，11，4096
        ...
        # return {"loss": loss, "qo_lhs": qo_lhs}
        return loss, qo_lhs
    ...
    def from_config(cls, cfg):
        ...
        llama_model = cfg.get("llama_model") # vicuna-7b
        ...
        prompt_path = cfg.get("prompt_path", "") # alignment.txt
        prompt_template = cfg.get("prompt_template", "")
        max_txt_len = cfg.get("max_txt_len", 32) # 160
