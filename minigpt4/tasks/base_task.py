    ...
    def _train_inner_loop(
        ...
        for i in metric_logger.log_every(range(my_iter), log_freq, header):
            ...
            samples.update(
                {
                    "epoch": inner_epoch,
                    "num_iters_per_epoch": iters_per_epoch,   # 1060
                    "iters": i,
                }
            )
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                loss, qo_lhs= self.train_step(model=model, samples=samples)

            ### L_kd ###
            if self.task_id > 0 and model.switch == False:
                # 冻结 model.pre_model 的参数
                for param in model.pre_model.parameters():
                    param.requires_grad = False

                fa = model.llama_proj(qo_lhs)
                fb = model.pre_model(qo_lhs)
            ...
            kk = 0
            if self.task_id > 0 and model.switch:
                for k, (m,params) in enumerate(model.named_parameters()):
                    if m == 'llama_proj.weight' and len(params.size()) != 1:
